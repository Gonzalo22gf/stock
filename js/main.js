...
let vistaActual = localStorage.getItem("vistaProductos") || "tabla";

let filtroFechaDias = 0;
...
buscarMovimiento?.addEventListener("input", () => { paginaMovimientos = 1; aplicarFiltroMovimientos(); });
filtroAccion?.addEventListener("change", () => { paginaMovimientos = 1; aplicarFiltroMovimientos(); });
...
document.addEventListener("click", (e) => {
  if (e.target.id === "btnLimpiarFiltrosMov") {
    ...
    if (fd) fd.value = "";
    if (fh) fh.value = "";
    filtroFechaDias = 0; filtroFechaDesdeVal = null; filtroFechaHastaVal = null;
...
function actualizarFechaTopbar() {
  ...
  if (topbarFecha) topbarFecha.textContent = new Date().toLocaleDateString("es-AR", { day: "2-digit", month: "2-digit", year: "numeric" });
}

function actualizarTopbarYSidebar() {
  if (!usuarioActivo) return;
  const t1 = document.querySelector("#topbarNombreUser");
  const t2 = document.querySelector("#topbarRolUser");
  const t3 = document.querySelector("#topbarAvatar");
  const t4 = document.querySelector("#topbarNombreSucursal");
  if (t1) t1.textContent = usuarioActivo.nombre;
  if (t2) t2.textContent = usuarioActivo.rol === "admin" ? "Administrador" : "Usuario";
  if (t3) t3.textContent = usuarioActivo.nombre?.[0]?.toUpperCase() || "U";
  if (t4) t4.textContent = usuarioActivo.rol === "admin" ? "Todas las sucursales" : (usuarioActivo.sucursal?.nombre || "Mi sucursal");
  const s1 = document.querySelector("#sidebarNombreUsuario");
  const s2 = document.querySelector("#sidebarRolUsuario");
  if (s1) s1.textContent = usuarioActivo.nombre;
  if (s2) s2.textContent = usuarioActivo.rol === "admin" ? "Administrador" : "Usuario";
}
...
function mostrarAuth() {
  ...
  if (sidebar) { sidebar.classList.add("oculto"); sidebar.classList.remove("open"); document.querySelector("#sidebarOverlay")?.classList.remove("show"); document.body.style.overflow = ""; }
  ocultarFAB();
...
function aplicarPermisosPorRol() {
  ...
  document.querySelectorAll(".solo-admin-section").forEach(el => {
    if (esAdmin) { el.classList.remove("oculto"); el.classList.remove("oculto-rol"); }
...
function crearSelectorSucursales() {
  ...
  if (!sel) return;
  sel.innerHTML = '<option value="">Todas las sucursales</option>' + sucursales.map(s => `<option value="${s._id}">${s.nombre}</option>`).join("");
  sel.value = sucursalSeleccionada || "";
  const newSel = sel.cloneNode(true);
  sel.parentNode.replaceChild(newSel, sel);
  newSel.addEventListener("change", async (e) => {
    sucursalSeleccionada = e.target.value;
    paginaMovimientos = 1; paginaProductos = 1;
    await cargarProductosAPI();
    await cargarResumenSucursales();
    await cargarMovimientosAPI();
  });
...
async function cargarSucursalesAPI() {
  try {
    const respuesta = await fetch(`${API_URL}/api/sucursales`, { headers: { Authorization: `Bearer ${token}` } });
...
// CAMBIO 2: RESUMEN SUCURSALES — mini-KPIs
async function cargarResumenSucursales() {
  if (usuarioActivo?.rol !== "admin") return;
  try {
    const respuesta = await fetch(`${API_URL}/api/sucursales/resumen`, { headers: { Authorization: `Bearer ${token}` } });
    const data = await respuesta.json();
    if (!respuesta.ok) throw new Error(data.mensaje || "Error al cargar resumen");
    resumenSucursales.innerHTML = "";
    if (data.length === 0) { resumenSucursales.innerHTML = `<p class="mensaje-vacio">No hay sucursales para mostrar.</p>`; return; }
    let resumenFinal;
    ...
    if (!resumenFinal) { resumenSucursales.innerHTML = `<p class="mensaje-vacio">No hay datos para esta sucursal.</p>`; return; }
    const valorFmt = resumenFinal.valorInventario.toLocaleString("es-AR", { style: "currency", currency: "ARS", maximumFractionDigits: 0 });
    resumenSucursales.innerHTML = `
      <div class="resumen-header">
        <span class="resumen-title">🏪 ${resumenFinal.sucursal.nombre}</span>
      </div>
      <div class="resumen-kpis">
        <div class="resumen-kpi">
          <span class="resumen-kpi-val">${resumenFinal.totalProductos}</span>
          <span class="resumen-kpi-lbl">Productos</span>
        </div>
        <div class="resumen-kpi resumen-kpi--warn">
          <span class="resumen-kpi-val">${resumenFinal.porVencer}</span>
          <span class="resumen-kpi-lbl">Por vencer</span>
        </div>
        <div class="resumen-kpi resumen-kpi--danger">
          <span class="resumen-kpi-val">${resumenFinal.vencidos}</span>
          <span class="resumen-kpi-lbl">Vencidos</span>
        </div>
        <div class="resumen-kpi resumen-kpi--purple">
          <span class="resumen-kpi-val">${resumenFinal.stockCritico}</span>
          <span class="resumen-kpi-lbl">Stock crítico</span>
        </div>
        <div class="resumen-kpi resumen-kpi--muted">
          <span class="resumen-kpi-val">${resumenFinal.agotados}</span>
          <span class="resumen-kpi-lbl">Agotados</span>
        </div>
        <div class="resumen-kpi resumen-kpi--green resumen-kpi--wide">
          <span class="resumen-kpi-val resumen-kpi-val--sm">${valorFmt}</span>
          <span class="resumen-kpi-lbl">Valor inventario</span>
        </div>
      </div>`;
  } catch (error) { console.error("ERROR RESUMEN SUCURSALES:", error); }
}
...
async function cargarMovimientosAPI() {
  ...
  try {
    ...
    if (usuarioActivo?.rol === "admin" && sucursalSeleccionada) url += `?sucursal=${sucursalSeleccionada}`;
...
function actualizarDashboardMovimientos(lista) {
  ...
  const movimientosHoy = lista.filter((m) => new Date(m.createdAt).toLocaleDateString("es-AR") === hoy);
...
function poblarFiltrosMovimientos(lista) {
  ...
  const valSuc = selectSuc.value; const valUsr = selectUsr.value;
  selectSuc.innerHTML = '<option value="todas">Todas las sucursales</option>' + sucursalesUnicas.map(s => `<option value="${s}">${s}</option>`).join("");
  selectUsr.innerHTML = '<option value="todos">Todos los usuarios</option>' + usuariosUnicos.map(u => `<option value="${u}">${u}</option>`).join("");
...
function aplicarFiltroMovimientos() {
  ...
  movimientosFiltradosActuales = movimientos.filter((m) => {
    const coincideTexto = (m.accion?.toLowerCase() || "").includes(texto) || (m.nombreProducto?.toLowerCase() || "").includes(texto) || (m.lote?.toLowerCase() || "").includes(texto) || (m.usuario?.nombre?.toLowerCase() || "").includes(texto) || (m.sucursal?.nombre?.toLowerCase() || "").includes(texto) || (m.detalle?.toLowerCase() || "").includes(texto);
    ...
    if (filtroFechaDias === -2 && (filtroFechaDesdeVal || filtroFechaHastaVal)) {
      if (filtroFechaDesdeVal) { const desde = new Date(filtroFechaDesdeVal); desde.setHours(0,0,0,0); if (fechaMov < desde) coincideFecha = false; }
      if (filtroFechaHastaVal) { const hasta = new Date(filtroFechaHastaVal); hasta.setHours(23,59,59,999); if (fechaMov > hasta) coincideFecha = false; }
    } else if (filtroFechaDias === 0) {
      const hoy = new Date(); hoy.setHours(0,0,0,0); coincideFecha = fechaMov >= hoy;
    } else if (filtroFechaDias > 0) {
      const limite = new Date(ahora - filtroFechaDias * 24 * 60 * 60 * 1000); coincideFecha = fechaMov >= limite;
    }
    const coincideSucursal = sucursalSelecMov === "todas" || (m.sucursal?.nombre || "") === sucursalSelecMov;
    const coincideUsuario  = usuarioSelecMov  === "todos"  || (m.usuario?.nombre  || "") === usuarioSelecMov;
...
function renderizarMovimientosPaginados() {
  ...
  if (paginaActualMovimientos) paginaActualMovimientos.textContent = `Página ${paginaMovimientos} de ${totalPaginas}`;
...
function paginaAnteriorMovimientos() { if (paginaMovimientos > 1) { paginaMovimientos--; renderizarMovimientosPaginados(); } }
function paginaSiguienteMovimientos() {
  const totalPaginas = Math.max(1, Math.ceil(movimientosFiltradosActuales.length / movimientosPorPagina));
  if (paginaMovimientos < totalPaginas) { paginaMovimientos++; renderizarMovimientosPaginados(); }
}

// CAMBIO 3: CARDS DE MOVIMIENTOS — compactas
function renderizarMovimientos(listaMovimientos) {
  ...
  if (!listaMovimientos || listaMovimientos.length === 0) {
    contenedorMovimientos.innerHTML = `<p class="mensaje-vacio">No hay movimientos para mostrar.</p>`; return;
  }
  listaMovimientos.forEach((movimiento) => {
    ...
    const iconos = { crear: "●", editar: "✎", eliminar: "✕" };
    const icono = iconos[claseAccion] || "●";
    card.innerHTML = `
      <div class="mov-row-top">
        <span class="accion-movimiento ${claseAccion}">${icono} ${movimiento.accion}</span>
        <span class="fecha-movimiento">${formatearFechaHora(movimiento.createdAt)}</span>
      </div>
      <div class="mov-nombre">${movimiento.nombreProducto}</div>
      <div class="mov-row-meta">
        ${movimiento.lote ? `<span class="mov-tag">Lote ${movimiento.lote}</span>` : ""}
        <span class="mov-tag">${movimiento.usuario?.nombre || "Sin datos"}</span>
        <span class="mov-tag">${movimiento.sucursal?.nombre || "Sin sucursal"}</span>
      </div>
      ${movimiento.detalle ? `<div class="mov-detalle">${movimiento.detalle}</div>` : ""}`;
...
function exportarMovimientos() {
  ...
  if (lista.length === 0) { Swal.fire({ icon: "info", title: "Sin movimientos", text: "No hay movimientos para exportar." }); return; }
  if (window.XLSX) { exportarMovimientosXLSX(lista); }
  else {
...
async function iniciarSesion(e) {
  e.preventDefault();
  const datosLogin = { email: document.querySelector("#loginEmail").value.trim(), password: document.querySelector("#loginPassword").value.trim() };
  try {
    const respuesta = await fetch(`${API_URL}/api/usuarios/login`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(datosLogin) });
    const data = await respuesta.json();
    if (!respuesta.ok) throw new Error(data.mensaje || "Error al iniciar sesión");
    guardarSesion(data); formLogin.reset(); mostrarApp();
    if (usuarioActivo.rol === "admin") { await cargarSucursalesAPI(); await cargarResumenSucursales(); }
    await cargarProductosAPI(); await cargarMovimientosAPI();
...
function guardarSesion(data) {
  token = data.token;
  usuarioActivo = { _id: data._id, nombre: data.nombre, email: data.email, rol: data.rol, sucursal: data.sucursal };
...
function cerrarSesion() {
  token = ""; usuarioActivo = null; productos = []; movimientos = []; movimientosFiltradosActuales = [];
  productosFiltradosActuales = []; sucursales = []; sucursalSeleccionada = "";
  paginaMovimientos = 1; paginaProductos = 1;
  localStorage.removeItem("tokenStockAlert"); localStorage.removeItem("usuarioStockAlert");
  const selectorAdmin = document.querySelector("#selectorSucursalAdmin");
  if (selectorAdmin) selectorAdmin.remove();
  ...
  if (buscarMovimiento) buscarMovimiento.value = "";
  if (filtroAccion) filtroAccion.value = "todos";
  actualizarDashboardMovimientos([]); renderizarProductos([]); actualizarResumen([]); mostrarAuth();
...
async function cargarProductosAPI() {
  ...
  try {
    ...
    renderizarProductos(productos); actualizarResumen(productos); mostrarAlertasVencimiento();
    actualizarGraficos(productos); actualizarPanelPremium(productos);
...
function normalizarProducto(producto) {
  return { ...producto, id: (producto._id || producto.id)?.toString(), vencimiento: producto.vencimiento?.split("T")[0] || producto.vencimiento, lote: producto.lote || "" };
}

function obtenerNombreSucursalProducto(producto) {
  if (producto.sucursal && typeof producto.sucursal === "object") return producto.sucursal.nombre || "Sin sucursal";
...
function renderizarProductosPaginados() {
  ...
  if (pagina.length === 0) {
    ...
    renderizarBotonesPaginacionProductos(totalPaginas); return;
  }
  ...
  if (vistaActual === "tabla") {
    ...
    tabla.innerHTML = `
      <thead>
        <tr>
          ...
          <th>Stock</th><th>Precio</th><th>Lote</th><th>Vencimiento</th><th>Estado</th><th>Acciones</th>
        </tr>
      </thead>
      <tbody>
        ${pagina.map((producto) => {
          ...
          return `
            <tr class="${claseBorde}" style="border-left:3px solid ${estado.clase === 'vencido' ? 'var(--red)' : estado.clase === 'por-vencer' ? 'var(--yellow)' : 'var(--green)'}">
              ...
              <td><span class="${estadoStock.clase === 'agotado' ? 'fecha-vencida' : estadoStock.clase === 'stock-critico' ? 'fecha-por-vencer' : ''}">${producto.stock}</span></td>
    ...
    // CAMBIO 1: CARDS — nueva versión compacta
    ...
    pagina.forEach((producto) => {
      ...
      card.innerHTML = `
        <div class="pc-header">
          <span class="pc-nombre">${producto.nombre}</span>
          ${usuarioActivo?.rol === "admin" ? `<span class="pc-sucursal" title="Sucursal ${nombreSucursalProducto}">${nombreSucursalProducto}</span>` : ""}
        </div>
        <div class="pc-meta">
          <span class="pc-cat">${producto.categoria}</span>
          ${producto.codigoBarras ? `<span class="pc-ean">${producto.codigoBarras}</span>` : ""}
        </div>
        <div class="pc-stats">
          <div class="pc-stat">
            <span class="pc-stat-val">${producto.stock}</span>
            <span class="pc-stat-lbl">Stock</span>
          </div>
          <div class="pc-stat">
            <span class="pc-stat-val">$${Number(producto.precio).toLocaleString("es-AR")}</span>
            <span class="pc-stat-lbl">Precio</span>
          </div>
          <div class="pc-stat">
            <span class="pc-stat-val pc-vence ${estado.clase === 'vencido' ? 'txt-red' : estado.clase === 'por-vencer' ? 'txt-yellow' : ''}">${formatearFecha(producto.vencimiento)}</span>
            <span class="pc-stat-lbl">Vence</span>
          </div>
        </div>
        <div class="pc-badges">
          <span class="estado ${estado.clase}">${estado.texto}</span>
          <span class="estado-stock ${estadoStock.clase}">${estadoStock.texto}</span>
          ${producto.lote ? `<span class="pc-lote-badge">Lote ${producto.lote}</span>` : ""}
        </div>`;

      const divBotones = document.createElement("div");
      divBotones.className = "botones";
      const btnEdit = document.createElement("button");
      btnEdit.className = "btn-editar"; btnEdit.textContent = "Editar"; btnEdit.type = "button";
      btnEdit.setAttribute("onclick", "editarProducto('" + pid + "')");
      divBotones.appendChild(btnEdit);
      const btnDel = document.createElement("button");
      btnDel.className = "btn-eliminar"; btnDel.textContent = "Eliminar"; btnDel.type = "button";
      ...
      if (usuarioActivo?.rol === "admin") {
        ...
        btnTrans.className = "btn-transferir"; btnTrans.textContent = "↗"; btnTrans.type = "button";
  ...
  if (info) info.textContent = total > 0 ? `Mostrando ${inicio + 1}–${Math.min(fin, total)} de ${total} productos` : "";
  aplicarClaseVista();
}
...
function renderizarBotonesPaginacionProductos(totalPaginas) {
  ...
  btnAnterior.textContent = "← Anterior"; btnAnterior.disabled = paginaProductos <= 1;
  btnAnterior.addEventListener("click", () => { if (paginaProductos > 1) { paginaProductos--; renderizarProductosPaginados(); scrollToProductos(); } });
  contenedor.appendChild(btnAnterior);
  const rango = 2;
  for (let i = 1; i <= totalPaginas; i++) {
    if (i === 1 || i === totalPaginas || (i >= paginaProductos - rango && i <= paginaProductos + rango)) {
      const btn = document.createElement("button"); btn.textContent = i;
      if (i === paginaProductos) btn.classList.add("activa");
      btn.addEventListener("click", () => { paginaProductos = i; renderizarProductosPaginados(); scrollToProductos(); });
      contenedor.appendChild(btn);
    } else if (i === paginaProductos - rango - 1 || i === paginaProductos + rango + 1) {
      const sep = document.createElement("span"); sep.textContent = "…"; sep.className = "pag-info"; contenedor.appendChild(sep);
    }
  }
  const btnSiguiente = document.createElement("button");
  btnSiguiente.textContent = "Siguiente →"; btnSiguiente.disabled = paginaProductos >= totalPaginas;
  btnSiguiente.addEventListener("click", () => { if (paginaProductos < totalPaginas) { paginaProductos++; renderizarProductosPaginados(); scrollToProductos(); } });
...
function obtenerEstadoProducto(fechaVencimiento) {
  const hoy = new Date(); const vencimiento = new Date(fechaVencimiento);
...
async function agregarProducto(e) {
  ...
  if (!nombre || !categoria || precio <= 0 || lotes.length === 0) {
    Swal.fire({ icon: "warning", title: "Datos incompletos", text: "Completá todos los campos correctamente." }); return;
  }
  ...
  if (usuarioActivo?.rol === "admin") {
    if (sucursalSeleccionada) { nuevoProducto.sucursal = sucursalSeleccionada; }
      ...
      const { value: sucursalElegida, isConfirmed } = await Swal.fire({
        ...
        html: `<select id="swalSucursal" class="swal2-input" style="width:85%;margin:0 auto;display:block"><option value="">Seleccioná una sucursal</option>${opcionesSuc}</select>`,
        confirmButtonText: "Confirmar", showCancelButton: true, cancelButtonText: "Cancelar",
        preConfirm: () => { const val = document.getElementById("swalSucursal").value; if (!val) { Swal.showValidationMessage("Seleccioná una sucursal"); return false; } return val; }
      });
  ...
  try {
    const respuesta = await fetch(`${API_URL}/api/productos`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify(nuevoProducto) });
    const data = await respuesta.json();
    if (!respuesta.ok) throw new Error(data.mensaje || "Error al agregar producto");
    productos.push(normalizarProducto(data)); aplicarFiltros();
    if (usuarioActivo?.rol === "admin") await cargarResumenSucursales();
    await cargarMovimientosAPI(); formProducto.reset();
  ...
  } catch (error) { console.error("ERROR PRODUCTO:", error); Swal.fire({ icon: "error", title: "Error", text: error.message }); }
}
...
function manejarClickProductos(e) {
  ...
  if (btn.classList.contains("btn-eliminar")) { e.stopPropagation(); eliminarProducto(id); }
  else if (btn.classList.contains("btn-editar")) { e.stopPropagation(); editarProducto(id); }
  else if (btn.classList.contains("btn-transferir")) { e.stopPropagation(); transferirProducto(id); }
}
...
async function eliminarProducto(idProducto) {
  ...
  .then(async (resultado) => {
    if (resultado.isConfirmed) {
      try {
        const respuesta = await fetch(`${API_URL}/api/productos/${idProducto}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
        const data = await respuesta.json();
        if (!respuesta.ok) throw new Error(data.mensaje || "Error al eliminar producto");
        productos = productos.filter((p) => p.id !== idProducto); aplicarFiltros();
        if (usuarioActivo?.rol === "admin") await cargarResumenSucursales();
        await cargarMovimientosAPI();
      ...
      } catch (error) { Swal.fire({ icon: "error", title: "Error", text: "No se pudo eliminar el producto." }); }
    }
  });
}
...
function editarProducto(idProducto) {
  ...
  if (!productoEncontrado) { Swal.fire({ icon:"error", title:"Error", text:"Producto no encontrado (id: "+idProducto+")" }); return; }
  Swal.fire({
    ...
    html: `
      ...
      <select id="editarCategoria" class="swal2-input">
        <option value="Lácteos">Lácteos</option><option value="Bebidas">Bebidas</option>
        <option value="Almacén">Almacén</option><option value="Limpieza">Limpieza</option><option value="Congelados">Congelados</option>
      </select>
    ...
    preConfirm: () => {
      ...
      if (!nombre || !categoria || stock < 0 || precio <= 0 || !vencimiento) { Swal.showValidationMessage("Completá todos los campos correctamente"); return false; }
      return { nombre, categoria, stock, precio, vencimiento, codigoBarras, lote };
    }
  }).then(async (resultado) => {
    if (resultado.isConfirmed) {
      try {
        const respuesta = await fetch(`${API_URL}/api/productos/${idProducto}`, { method: "PUT", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify(resultado.value) });
        const data = await respuesta.json();
        if (!respuesta.ok) throw new Error(data.mensaje || "Error al actualizar producto");
        productos = productos.map((p) => p.id === idProducto ? normalizarProducto(data) : p); aplicarFiltros();
        if (usuarioActivo?.rol === "admin") await cargarResumenSucursales();
        await cargarMovimientosAPI();
        Swal.fire({ icon: "success", title: "Producto actualizado", timer: 1500, showConfirmButton: false });
      } catch (error) { Swal.fire({ icon: "error", title: "Error", text: "No se pudo actualizar el producto." }); }
    }
  });
}
...
function aplicarFiltros() {
  ...
  let productosFiltrados = productos.filter((producto) => {
    const coincideBusqueda = producto.nombre.toLowerCase().includes(textoBusqueda) || (producto.codigoBarras || "").toLowerCase().includes(textoBusqueda) || (producto.lote || "").toLowerCase().includes(textoBusqueda);
  ...
  renderizarProductos(productosFiltrados); actualizarResumen(productos);
}

function limpiarFiltros() {
  buscador.value = ""; filtroCategoria.value = "todas"; filtroEstado.value = "todos"; ordenar.value = "";
  renderizarProductos(productos); actualizarResumen(productos);
...
function actualizarResumen(listaProductos) {
  ...
  valorInventario.textContent = valorTotal.toLocaleString("es-AR", { style: "currency", currency: "ARS", maximumFractionDigits: 0 });
}
...
function mostrarAlertasVencimiento() {
  ...
  function buildPreview(lista, color, etiqueta) {
    ...
    const masItems = resto > 0 ? `<p style="font-size:.78rem;color:#94a3b8;margin-top:6px;text-align:right;">+ ${resto} más...</p>` : "";
    return `
      <div style="margin-bottom:14px;text-align:left;">
        ...
        <div style="background:#f8fafc;border-radius:10px;padding:4px 12px;">${items}</div>
        ${masItems}
      </div>`;
  }
  const html = `
    <div style="text-align:left;">
      ...
      <p style="font-size:.75rem;color:#94a3b8;text-align:center;margin-top:4px;">Revisá el panel de alertas para ver el listado completo</p>
    </div>`;
  Swal.fire({
    title: `🚨 ${totalAlertas} producto${totalAlertas !== 1 ? "s" : ""} requieren atención`,
    html, confirmButtonText: "Ver alertas", showCancelButton: true, cancelButtonText: "Cerrar",
    width: Math.min(window.innerWidth - 32, 480),
    customClass: { popup: "swal-alerta-compacta", title: "swal-alerta-titulo" }
  });
}
...
function aplicarModoGuardado() {
  if (JSON.parse(localStorage.getItem("modoClaro") || "false")) document.body.classList.add("light");
}

async function iniciarEscaner() {
  if (escanerActivo) { await detenerEscaner(); return; }
  const lectorEl = document.getElementById("lectorCodigo");
  if (!lectorEl) { Swal.fire({ icon: "error", title: "Error", text: "No se encontró el elemento del escáner." }); return; }
  lectorEl.classList.remove("oculto");
  html5QrCode = new Html5Qrcode("lectorCodigo");
  try {
    await html5QrCode.start(
      { facingMode: "environment" }, { fps: 10, qrbox: { width: 250, height: 120 } },
      async (codigoDetectado) => {
        codigoBarrasInput.value = codigoDetectado; await detenerEscaner();
        Swal.fire({ icon: "success", title: "EAN detectado", text: codigoDetectado, timer: 1500, showConfirmButton: false });
      }
    );
    escanerActivo = true;
    if (btnEscanear) btnEscanear.innerHTML = `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="3" y1="15" x2="21" y2="15"/><line x1="9" y1="3" x2="9" y2="21"/><line x1="15" y1="3" x2="15" y2="21"/></svg> Detener escáner`;
  } catch (error) {
...
async function detenerEscaner() {
  if (html5QrCode && escanerActivo) { await html5QrCode.stop(); await html5QrCode.clear(); }
  const lectorElStop = document.getElementById("lectorCodigo");
  if (lectorElStop) lectorElStop.classList.add("oculto");
  escanerActivo = false; html5QrCode = null;
  if (btnEscanear) btnEscanear.innerHTML = `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="3" y1="15" x2="21" y2="15"/><line x1="9" y1="3" x2="9" y2="21"/><line x1="15" y1="3" x2="15" y2="21"/></svg> Escanear EAN`;
}

function exportarExcel() {
  if (productos.length === 0) { Swal.fire({ icon: "info", title: "Sin productos", text: "No hay productos para exportar." }); return; }
  if (window.XLSX) { exportarXLSX(); }
  else {
    const script = document.createElement("script");
    script.src = "https://cdn.jsdelivr.net/npm/xlsx@0.18.5/dist/xlsx.full.min.js";
    script.onload = exportarXLSX; script.onerror = exportarCSVCompatible;
    document.head.appendChild(script);
  }
}
...
function exportarXLSX() {
  ...
  const filas = productos.map((p) => [
    p.nombre, p.lote || "", p.categoria, Number(p.stock), Number(p.precio), p.codigoBarras || "",
    formatearFecha(p.vencimiento), obtenerEstadoProducto(p.vencimiento).texto, obtenerEstadoStock(p.stock).texto,
    obtenerNombreSucursalProducto(p), obtenerNombreUsuario(p.creadoPor), obtenerNombreUsuario(p.actualizadoPor),
...
function exportarCSVCompatible() {
  ...
  productos.forEach((p) => {
    const fila = [p.nombre, p.lote || "", p.categoria, p.stock, p.precio, p.codigoBarras || "", formatearFecha(p.vencimiento),
      obtenerEstadoProducto(p.vencimiento).texto, obtenerEstadoStock(p.stock).texto, obtenerNombreSucursalProducto(p),
      obtenerNombreUsuario(p.creadoPor), obtenerNombreUsuario(p.actualizadoPor), formatearFechaHora(p.fechaUltimaActualizacion || p.updatedAt)
  ...
  enlace.href = url; enlace.download = `StockAlert-Inventario-${Date.now()}.csv`; enlace.click();
...
function importarExcel() {
  ...
  if (!archivo) { Swal.fire({ icon: "warning", title: "Seleccioná un archivo", text: "Primero elegí un archivo CSV para importar." }); return; }
  ...
  lector.onload = async function (e) {
    ...
    const productosImportados = lineas.slice(1).map((linea) => {
      ...
      const productoFinal = {
        nombre: producto.producto || producto.nombre, lote: producto.lote || "",
        categoria: producto.categoría || producto.categoria, stock: Number(producto.stock), precio: Number(producto.precio),
        codigoBarras: producto.ean || producto.codigobarras || "", vencimiento: convertirFechaImportada(producto.vencimiento)
      };
      if (usuarioActivo?.rol === "admin" && sucursalSeleccionada) productoFinal.sucursal = sucursalSeleccionada;
      return productoFinal;
    });
    const productosValidos = productosImportados.filter((p) => p.nombre && p.categoria && !Number.isNaN(p.stock) && !Number.isNaN(p.precio) && p.vencimiento);
    if (productosValidos.length === 0) { Swal.fire({ icon: "error", title: "Archivo inválido", text: "No se encontraron productos válidos para importar." }); return; }
    try {
      for (const producto of productosValidos) {
        const respuesta = await fetch(`${API_URL}/api/productos`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify(producto) });
        const data = await respuesta.json();
        if (!respuesta.ok) throw new Error(data.mensaje || "Error al importar producto");
        productos.push(normalizarProducto(data));
      }
      aplicarFiltros(); actualizarResumen(productos);
      if (usuarioActivo?.rol === "admin") await cargarResumenSucursales();
      await cargarMovimientosAPI();
      inputImportar.value = "";
    ...
    } catch (error) { Swal.fire({ icon: "error", title: "Error al importar", text: error.message }); }
  };
...
function convertirFechaImportada(fecha) {
  ...
  if (fecha.includes("/")) { const partes = fecha.split("/"); if (partes.length === 3) return `${partes[2]}-${partes[1].padStart(2,"0")}-${partes[0].padStart(2,"0")}`; }
  return fecha;
}
...
function actualizarGraficos(listaProductos) {
  ...
  if (graficoEstados) {
    chartEstados = new Chart(graficoEstados, { type: "doughnut", data: { labels: ["En buen estado", "Por vencer", "Vencidos"], datasets: [{ data: [totalOk, totalPorVencer, totalVencidos], backgroundColor: coloresEstado, borderWidth: 2, borderColor: "#fff" }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: "top", labels: { font: { size: 11 } } } } } });
  }
  if (graficoCategorias) {
    chartCategorias = new Chart(graficoCategorias, { type: "bar", data: { labels: nombresCategorias, datasets: [{ label: "Productos", data: nombresCategorias.map((c) => categorias[c].cantidad), backgroundColor: colorBarra, borderRadius: 6 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { labels: { font: { size: 11 } } } }, scales: { y: { beginAtZero: true, grid: { color: "#f1f5f9" } }, x: { grid: { display: false } } } } });
  }
  if (graficoStockCategorias) {
    chartStockCategorias = new Chart(graficoStockCategorias, { type: "bar", data: { labels: nombresCategorias, datasets: [{ label: "Stock", data: nombresCategorias.map((c) => categorias[c].stock), backgroundColor: "#06b6d4", borderRadius: 6 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { labels: { font: { size: 11 } } } }, scales: { y: { beginAtZero: true, grid: { color: "#f1f5f9" } }, x: { grid: { display: false } } } } });
  }
  if (graficoValorInventario) {
    chartValorInventario = new Chart(graficoValorInventario, { type: "bar", data: { labels: nombresCategorias, datasets: [{ label: "Valor inventario", data: nombresCategorias.map((c) => categorias[c].valor), backgroundColor: "#22c55e", borderRadius: 6 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { labels: { font: { size: 11 } } } }, scales: { y: { beginAtZero: true, grid: { color: "#f1f5f9" } }, x: { grid: { display: false } } } } });
  }
}
...
function actualizarPanelPremium(listaProductos) {
  ...
  const criticos = listaProductos.filter((p) => { const s = obtenerEstadoStock(Number(p.stock || 0)).clase; return s === "stock-critico" || s === "agotado"; });
  ...
  productosEnRiesgo.forEach((p) => { const nombre = obtenerNombreSucursalProducto(p) || "Sin sucursal"; conteoSucursales[nombre] = (conteoSucursales[nombre] || 0) + 1; });
  ...
  if (badge) { badge.textContent = totalAlertas; badge.classList.toggle("visible", totalAlertas > 0); }
  const urgentes = [...vencidos, ...porVencer].sort((a, b) => new Date(a.vencimiento) - new Date(b.vencimiento)).slice(0, 10);
  ...
  listaProductos.forEach((p) => {
    ...
    if (!productosUnicos.some((u) => `${u.nombre}-${u.lote || ""}-${u.vencimiento}` === clave)) productosUnicos.push(p);
  });
  ...
  productosRiesgosos.forEach((producto, index) => {
    let claseNum = "";
    if (index === 0) claseNum = "oro"; else if (index === 1) claseNum = "plata"; else if (index === 2) claseNum = "bronce";
...
function mostrarSkeleton(mostrar) {
  ...
  if (!skeleton) return;
  if (mostrar) { skeleton.classList.remove("oculto"); if (contenedor) contenedor.innerHTML = ""; if (paginacion) paginacion.innerHTML = ""; }
  else { skeleton.classList.add("oculto"); }
}
...
function iniciarFiltroFechas() {
  document.addEventListener("click", (e) => {
    if (e.target.classList.contains("fecha-chip")) {
      ...
      filtroFechaDesdeVal = null; filtroFechaHastaVal = null;
      const fd = document.querySelector("#filtroFechaDesde"); const fh = document.querySelector("#filtroFechaHasta");
      if (fd) fd.value = ""; if (fh) fh.value = "";
      paginaMovimientos = 1; aplicarFiltroMovimientos();
    }
    if (e.target.id === "btnLimpiarFechas") {
      filtroFechaDesdeVal = null; filtroFechaHastaVal = null; filtroFechaDias = -1;
      const fd = document.querySelector("#filtroFechaDesde"); const fh = document.querySelector("#filtroFechaHasta");
      if (fd) fd.value = ""; if (fh) fh.value = "";
      document.querySelectorAll(".fecha-chip").forEach(c => c.classList.remove("activa"));
  ...
  document.addEventListener("change", (e) => {
    if (e.target.id === "filtroFechaDesde") { filtroFechaDesdeVal = e.target.value; filtroFechaDias = -2; document.querySelectorAll(".fecha-chip").forEach(c => c.classList.remove("activa")); paginaMovimientos = 1; aplicarFiltroMovimientos(); }
    if (e.target.id === "filtroFechaHasta") { filtroFechaHastaVal = e.target.value; filtroFechaDias = -2; document.querySelectorAll(".fecha-chip").forEach(c => c.classList.remove("activa")); paginaMovimientos = 1; aplicarFiltroMovimientos(); }
  });
}
...
const SECCIONES_MAP = {
  ...
  movimientos: ["#seccionHistorial"], reportes: ["#dashboardGraficos"],
  importar: [".card:has(#btnExportarExcel)"], escaner: ["#lectorCodigo"],
  sucursales: ["#panelAdminGlobal"], cuenta: ["#seccionUsuario"],
};

function navegarSeccion(seccion) {
  const targets = {
    dashboard: document.querySelector("#panelAdminGlobal") || document.querySelector(".kpi-row"),
    productos: document.querySelector("#seccionProductos"), movimientos: document.querySelector("#seccionHistorial"),
    reportes: document.querySelector("#dashboardGraficos"), importar: document.querySelector("#btnExportarExcel")?.closest(".card"),
    escaner: document.querySelector("#btnEscanear")?.closest(".card"), sucursales: document.querySelector("#panelAdminGlobal"),
    cuenta: document.querySelector("#seccionUsuario"),
  };
  const el = targets[seccion];
  if (el) el.scrollIntoView({ behavior: "smooth", block: "start" });
  if (seccion === "escaner") document.querySelector("#btnEscanear")?.click();
...
function iniciarNavSidebar() {
  document.querySelectorAll(".nav-item[data-section]").forEach(item => {
    item.addEventListener("click", (e) => {
      ...
      if (window.innerWidth <= 900) { document.querySelector("#sidebar")?.classList.remove("open"); document.querySelector("#sidebarOverlay")?.classList.remove("show"); document.body.style.overflow = ""; }
    });
  });
}
...
function iniciarSidebarCollapse() {
  ...
  if (savedCollapsed) { sidebar.classList.add("collapsed"); document.body.classList.add("sidebar-collapsed"); }
  ...
  document.querySelectorAll(".nav-item").forEach(a => {
    a.addEventListener("click", () => {
      if (window.innerWidth <= 900) { sidebar.classList.remove("open"); overlay?.classList.remove("show"); document.body.style.overflow = ""; }
...
function iniciarBusquedaGlobal() {
  ...
  input.addEventListener("input", () => {
    ...
    if (!texto || texto.length < 2) { resultados.innerHTML = '<p class="busqueda-hint">Escribí para buscar en todo el inventario</p>'; return; }
    if (!productos || productos.length === 0) { resultados.innerHTML = '<p class="busqueda-vacio">No hay productos cargados</p>'; return; }
    const encontrados = productos.filter((p) =>
      p.nombre.toLowerCase().includes(texto) || (p.codigoBarras || "").toLowerCase().includes(texto) ||
      (p.lote || "").toLowerCase().includes(texto) || (p.categoria || "").toLowerCase().includes(texto)
    ).slice(0, 8);
    if (encontrados.length === 0) { resultados.innerHTML = '<p class="busqueda-vacio">Sin resultados para "' + texto + '"</p>'; return; }
...
function irAProducto(id) {
  ...
  aplicarFiltros();
  setTimeout(() => { const sec = document.querySelector("#seccionProductos"); if (sec) sec.scrollIntoView({ behavior: "smooth", block: "start" }); }, 100);
}
...
function calcularTendencia(actual, anterior, elementoId) {
  ...
  if (actual > anterior) { el.textContent = `↑ +${actual - anterior} vs antes`; el.className = "kpi-tendencia sube"; }
  else if (actual < anterior) { el.textContent = `↓ -${anterior - actual} vs antes`; el.className = "kpi-tendencia baja"; }
  else { el.textContent = "= Sin cambios"; el.className = "kpi-tendencia igual"; }
}

function exportarMovimientosXLSX(lista) {
  const XLSX = window.XLSX;
  const encabezados = ["Fecha","Acción","Producto","Lote","Usuario","Sucursal","Detalle"];
  const filas = lista.map((m) => [formatearFechaHora(m.createdAt), m.accion, m.nombreProducto, m.lote || "Sin lote", m.usuario?.nombre || "Sin datos", m.sucursal?.nombre || "Sin sucursal", m.detalle || "Sin detalle"]);
  const hoja = XLSX.utils.aoa_to_sheet([encabezados, ...filas]);
...
function exportarMovimientosCSV(lista) {
  ...
  lista.forEach((m) => { csv += [formatearFechaHora(m.createdAt), m.accion, m.nombreProducto, m.lote || "Sin lote", m.usuario?.nombre || "Sin datos", m.sucursal?.nombre || "Sin sucursal", m.detalle || "Sin detalle"].join("\t") + "\n"; });
  const blob = new Blob([csv], { type: "text/tab-separated-values;charset=utf-8;" });
  const url = URL.createObjectURL(blob);
...
async function cargarUsuariosAdmin() {
  ...
  try {
    const respuesta = await fetch(`${API_URL}/api/usuarios`, { headers: { Authorization: `Bearer ${token}` } });
    if (respuesta.status === 404) {
      contenedor.innerHTML = `<div class="admin-aviso"><p>⚠️ El endpoint <code>/api/usuarios</code> no está disponible en el backend.</p><p>Podés gestionar las sucursales desde la sección de abajo:</p></div>`;
      await cargarGestionSucursales(contenedor); return;
    }
    const data = await respuesta.json();
    if (!respuesta.ok) throw new Error(data.mensaje || "Error");
    if (data.length === 0) { contenedor.innerHTML = `<p class="mensaje-vacio">No hay usuarios registrados.</p>`; await cargarGestionSucursales(contenedor); return; }
    contenedor.innerHTML = `
      <div class="tabla-usuarios-wrap">
        <table class="tabla-admin">
          <thead><tr><th>Usuario</th><th>Email</th><th>Sucursal</th><th>Rol</th><th>Estado</th><th>Acciones</th></tr></thead>
          <tbody>
            ${data.map((u) => `
              <tr class="${u.activo === false ? "usuario-inactivo" : ""}">
                <td><strong>${u.nombre}</strong></td>
                <td>${u.email}</td>
                <td><span>${u.sucursal?.nombre || "Sin sucursal"}</span>${u.sucursal?._id ? `<button class="btn-rename-suc" onclick="editarSucursal('${u.sucursal._id}')">✏️</button>` : ""}</td>
                <td>
...
async function cargarGestionSucursales(contenedor) {
  ...
  if (!sucursales || sucursales.length === 0) {
    try { const respuesta = await fetch(`${API_URL}/api/sucursales`, { headers: { Authorization: `Bearer ${token}` } }); const data = await respuesta.json(); if (respuesta.ok) sucursales = data; } catch(e) {}
  }
  if (!sucursales || sucursales.length === 0) { const div = document.createElement("div"); div.innerHTML = `<p class="mensaje-vacio">No hay sucursales registradas.</p>`; contenedor.appendChild(div); return; }
  const div = document.createElement("div");
  div.innerHTML = `
    <h3 style="font-size:.85rem;font-weight:700;color:var(--text-1);margin-bottom:12px;padding-top:16px;border-top:1px solid var(--border)">🏪 Gestión de sucursales</h3>
    <div class="buscador-sucursales-wrap"><input type="text" class="buscador-sucursales" id="buscadorSucursales" placeholder="Buscar por nombre o número de sucursal..." autocomplete="off" spellcheck="false"></div>
      ...
      ${sucursales.map((s) => `
        <div class="sucursal-card" data-nombre="${s.nombre.toLowerCase()}" data-numero="${s.numero || s.nro || ""}">
          <div class="sucursal-card-header">
            ...
            <div class="sucursal-info"><span class="sucursal-nombre">${s.nombre}</span><small class="sucursal-dir">${s.direccion || "Sin dirección registrada"}</small></div>
            <button class="btn-editar-sucursal" onclick="editarSucursal('${s._id}')" title="Editar sucursal">
  ...
  buscadorSuc?.addEventListener("input", () => {
    ...
    div.querySelectorAll(".sucursal-card").forEach((card) => {
      const nombre = card.dataset.nombre || ""; const numero = card.dataset.numero || "";
      card.classList.toggle("oculta", !nombre.includes(texto) && !numero.includes(texto));
    });
  });
}
...
async function cambiarRolUsuario(idUsuario, nuevoRol) {
  try {
    const respuesta = await fetch(`${API_URL}/api/usuarios/${idUsuario}/rol`, { method: "PUT", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ rol: nuevoRol }) });
    const data = await respuesta.json();
  ...
  } catch (error) { Swal.fire({ icon: "error", title: "Error", text: error.message }); await cargarUsuariosAdmin(); }
}

async function toggleActivarUsuario(idUsuario, estaActivo) {
  const accion = estaActivo ? "desactivar" : "activar";
  const resultado = await Swal.fire({ title: `¿${accion.charAt(0).toUpperCase() + accion.slice(1)} usuario?`, text: estaActivo ? "El usuario no podrá iniciar sesión." : "El usuario podrá volver a iniciar sesión.", icon: "warning", showCancelButton: true, confirmButtonText: `Sí, ${accion}`, cancelButtonText: "Cancelar" });
  if (!resultado.isConfirmed) return;
  try {
    const respuesta = await fetch(`${API_URL}/api/usuarios/${idUsuario}/estado`, { method: "PUT", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ activo: !estaActivo }) });
    const data = await respuesta.json();
    if (!respuesta.ok) throw new Error(data.mensaje || "Error");
...
async function cargarSucursalesAdmin() {
  ...
  try {
    ...
    if (!sucursalesData || sucursalesData.length === 0) {
      const resp = await fetch(`${API_URL}/api/sucursales`, { headers: { Authorization: `Bearer ${token}` } });
    ...
    contenedor.innerHTML = `
      <div class="tabla-usuarios-wrap">
        <table class="tabla-admin">
          <thead><tr><th>N°</th><th>Nombre</th><th>Dirección</th><th>Productos</th><th>Vencidos</th><th>Stock crítico</th><th>Valor inventario</th><th>Acciones</th></tr></thead>
          <tbody>
            ${sucursalesData.map((s) => {
              ...
              return `
                <tr>
                  ...
                  <td style="color:${(resumen.vencidos || 0) > 0 ? "var(--red)" : "var(--text-3)"}">${resumen.vencidos ?? "—"}</td>
                  <td style="color:${(resumen.stockCritico || 0) > 0 ? "var(--orange)" : "var(--text-3)"}">${resumen.stockCritico ?? "—"}</td>
                  <td>${resumen.valorInventario != null ? resumen.valorInventario.toLocaleString("es-AR", { style: "currency", currency: "ARS", maximumFractionDigits: 0 }) : "—"}</td>
                  <td><button class="btn-edit-suc" onclick="editarSucursal('${s._id}')" title="Editar sucursal">✏️ Editar</button></td>
                </tr>`;
            }).join("")}
          </tbody>
        </table>
      </div>
      <p style="font-size:.7rem;color:var(--text-4);margin-top:10px;padding:8px 12px;background:rgba(99,102,241,.08);border-radius:8px;border-left:3px solid var(--brand)">💡 Hacé clic en <strong>✏️ Editar</strong> para cambiar el nombre, número o dirección de una sucursal.</p>`;
  } catch (error) {
    contenedor.innerHTML = `<div style="text-align:center;padding:24px"><p style="color:var(--text-4);font-size:.85rem">No se pudieron cargar las sucursales.</p></div>`;
  }
}
...
async function editarSucursal(idSucursal) {
  ...
  if (!sucursal) { Swal.fire({ icon: "error", title: "Error", text: "Sucursal no encontrada." }); return; }
...
async function cargarComparativoSucursales() {
  ...
  try {
    ...
    if (!data || data.length === 0) { contenedor.innerHTML = `<p class="mensaje-vacio">Sin datos de sucursales.</p>`; return; }
        ...
        ${ranking.map((s, i) => {
          ...
          return `
            <div class="card-comparativo">
              ...
              <div class="comparativo-barra"><div class="barra-fill" style="width:${Math.min(100, riesgo * 10)}%;background:${colorRiesgo}"></div></div>
            </div>`;
        }).join("")}
      </div>`;
  } catch (error) { contenedor.innerHTML = `<p class="mensaje-vacio">No se pudo cargar el comparativo.</p>`; }
}

async function transferirProducto(idProducto) {
  if (sucursales.length === 0) { Swal.fire({ icon: "warning", title: "Sin sucursales", text: "No hay sucursales disponibles." }); return; }
  const producto = productos.find((p) => p.id === idProducto);
  if (!producto) return;
  const opcionesSucursales = sucursales.filter((s) => s._id !== (producto.sucursal?._id || producto.sucursal)).map((s) => `<option value="${s._id}">${s.nombre}</option>`).join("");
  const resultado = await Swal.fire({
    title: `Transferir "${producto.nombre}"`,
    html: `<p style="margin-bottom:12px;color:#64748b;font-size:.9rem;">Seleccioná la sucursal destino</p><select id="sucursalDestino" class="swal2-input" style="width:85%;margin:0 auto;display:block">${opcionesSucursales}</select>`,
    showCancelButton: true, confirmButtonText: "Transferir", cancelButtonText: "Cancelar",
    preConfirm: () => { const destino = document.querySelector("#sucursalDestino").value; if (!destino) { Swal.showValidationMessage("Seleccioná una sucursal"); return false; } return destino; }
  });
  if (!resultado.isConfirmed) return;
  try {
    const bodyTransfer = {
      nombre: producto.nombre, categoria: producto.categoria, precio: producto.precio, stock: producto.stock,
      vencimiento: producto.vencimiento, codigoBarras: producto.codigoBarras || "", lote: producto.lote || "",
      sucursal: resultado.value,
      lotes: (producto.lotes && producto.lotes.length > 0) ? producto.lotes : [{ numero: producto.lote || "", stock: producto.stock, vencimiento: producto.vencimiento }]
    };
    const respuesta = await fetch(`${API_URL}/api/productos/${idProducto}`, { method: "PUT", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify(bodyTransfer) });
    const data = await respuesta.json();
    ...
    aplicarFiltros();
    await cargarResumenSucursales(); await cargarComparativoSucursales(); await cargarMovimientosAPI();
...
async function abrirConfigAlertas() {
  ...
  const resultado = await Swal.fire({
    ...
    html: `
      <div style="text-align:left;padding:0 8px">
        <label style="display:block;font-weight:700;margin-bottom:6px;font-size:.88rem;color:#334155">📅 Días de anticipación para alertar vencimientos</label>
        <input id="diasAlerta" type="number" min="1" max="60" value="${diasActual}" class="swal2-input" style="width:100%;margin:0 0 16px">
        <label style="display:block;font-weight:700;margin-bottom:6px;font-size:.88rem;color:#334155">📦 Umbral de stock bajo (unidades)</label>
        <input id="umbralStock" type="number" min="1" max="100" value="${stockActual}" class="swal2-input" style="width:100%;margin:0">
      </div>`,
  ...
  if (productos.length > 0) { renderizarProductos(productos); actualizarResumen(productos); actualizarPanelPremium(productos); }
}
...
function obtenerEstadoProductoConf(fechaVencimiento) {
  ...
  const hoy = new Date(); const venc = new Date(fechaVencimiento);
...
let fabSucursalFija = null;

function mostrarFAB() { const fab = document.getElementById("fabScanner"); if (fab) fab.classList.add("visible"); }
function ocultarFAB() { const fab = document.getElementById("fabScanner"); if (fab) fab.classList.remove("visible"); }
...
function iniciarFAB() {
  ...
  document.getElementById("fabModalOverlay")?.addEventListener("click", (e) => { if (e.target.id === "fabModalOverlay") cerrarFabModal(); });
...
async function iniciarFabScanner() {
  ...
  try {
    ...
    await fabQrCode.start(
      { facingMode: "environment" }, { fps: 15, qrbox: { width: 260, height: 110 } },
      async (codigoDetectado) => {
  ...
  } catch (error) { Swal.fire({ icon: "error", title: "Error de cámara", text: "No se pudo acceder a la cámara." }); cerrarFabModal(); }
}

async function detenerFabScanner() {
  if (fabQrCode && fabEscanerActivo) { try { await fabQrCode.stop(); await fabQrCode.clear(); } catch(e) {} }
  fabEscanerActivo = false; fabQrCode = null;
}

async function reiniciarFabEscaner() { resetFabCampos(); await iniciarFabScanner(); }
...
async function guardarProductoFAB() {
  ...
  if (!nombre || !categoria || !precio || !vencimiento) { Swal.fire({ icon: "warning", title: "Datos incompletos", text: "Completá nombre, categoría, precio y vencimiento." }); return; }
  const nuevoProducto = { nombre, categoria, precio, codigoBarras: ean, lote, stock, vencimiento, lotes: [{ numero: lote, stock, vencimiento }] };
  if (usuarioActivo?.rol === "admin") {
    if (fabSucursalFija) { nuevoProducto.sucursal = fabSucursalFija; }
    else if (sucursales.length > 0) {
      const opcs = sucursales.map(s => `<option value="${s._id}">${s.nombre}</option>`).join("");
      const { value: suc, isConfirmed } = await Swal.fire({
        title: "¿A qué sucursal?", html: `<select id="swalSucFab" class="swal2-input" style="width:85%;margin:0 auto;display:block"><option value="">Seleccioná</option>${opcs}</select>`,
        confirmButtonText: "Confirmar", showCancelButton: true,
        preConfirm: () => { const v = document.getElementById("swalSucFab").value; if (!v) { Swal.showValidationMessage("Seleccioná una sucursal"); return false; } return v; }
      });
      if (!isConfirmed) return;
      nuevoProducto.sucursal = suc; fabSucursalFija = suc;
    }
  }
  ...
  try {
    ...
    btn.style.background = "var(--green-bg)"; btn.style.color = "var(--green)";
    setTimeout(async () => {
      btn.textContent = "Guardar producto"; btn.style.background = ""; btn.style.color = ""; btn.disabled = false;
      await reiniciarFabEscaner();
    }, 1200);
  } catch (error) { Swal.fire({ icon: "error", title: "Error", text: error.message }); btn.textContent = "Guardar producto"; btn.disabled = false; }
}